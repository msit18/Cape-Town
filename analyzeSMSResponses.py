...
class CitivanSMS:
	...
	def rateBus(self, rateBusName, currentConvoNum):
		...
		if (rateBusName in self.busJson == False) | (self.busJson[rateBusName]["12"] == 0):
			print "BUS HAS NOT BEEN RATED YET: ", (rateBusName in self.busJson == False)
			print "12 value is 0. Surveyors have not finished survey. ", self.busJson[rateBusName]["12"] == 0
	...
	def inputDataToCloudant(self, currentConvoNum, userText, surveyNum):
		...
		if currentConvoNum == 1:
			...
			self.json[self.callerID][strSurveyNum]["LastSubmitTime"] = strftime("%a, %d %b %Y %H:%M:%S +0000", gmtime())
